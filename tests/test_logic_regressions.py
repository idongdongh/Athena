...
class LogicRegressionTests(unittest.TestCase):
    ...
    def test_anthropic_client_uses_shared_timeout_and_disables_sdk_retries(self):
        ...
        self.assertEqual(
            agent.client.timeout.connect,
            conversation_loop.API_CONNECT_TIMEOUT,
        )
        self.assertEqual(
            agent.client.timeout.read,
    ...
    def test_guardrail_halt_returns_controlled_response_without_summary(self):
        ...
        previous_executor_guardrail = tool_executor.tool_guardrails
        configured_guardrail = ToolCallGuardrailController(
            ToolCallGuardrailConfig(hard_stop_enabled=True)
        )
        tool_executor.tool_guardrails = configured_guardrail
        try:
            with patch.object(conversation_loop, "client", fake_client):
                conversation_loop.agent_loop(messages)
        finally:
            tool_executor.tool_guardrails = previous_executor_guardrail
            if previous is None:
